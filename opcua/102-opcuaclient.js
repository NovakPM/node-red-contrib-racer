...
module.exports = function (RED) {
  ...
  function OpcUaClientNode(n) {
    ...
    // console.log("endpoint1", opcuaEndpoint);
    ...
    // DO NOT USE must be NodeNovakon-Client !! connectionOption.applicationName = node.name; // Application name
   ...
   async function getBrowseName(_session, nodeId, callback) {
    ...
    function readmultiple_action_input(msg) {
      ...
      if (msg.topic !== "readmultiple" && msg.topic !== "clearitems" && msg.topic !== "additems") {
        if (item.length > 0) {
      ...
      console.log("multipleItems", msg.multipleItems !== undefined, msg.topic === "additems")
      if(msg.multipleItems !== undefined && msg.topic === "additems") {
        multipleItems = [];
        for (item of msg.multipleItems) {
          multipleItems.push(item.nodeId);
        }
      }
    ...
    function write_action_input(msg) {
      ...
      if (node.session) {
        ...
        // console.log("nodeToWrite",nodeToWrite);
        node.session.write(nodeToWrite, function (err) {
          if (err) {
            ...
            reset_opcua_client(connect_opcua_client);
            msg.payload = node.name + " Cannot write value (" + msg.payload + ") to msg.topic:" + msg.topic + " error:" + err;
          } else {
            set_node_status_to("value written");
            verbose_log("Value written!");
            msg.payload = "Value " + opcuaDataValue.value + " written!";
          }
          node.send(msg);
        });
      } else {
        set_node_status_to("Session invalid");
        node_error("Session is not active!")
        msg.payload = "Session is not active!";
        node.send(msg);
      }
    }
    ...
    function subscribe_action_input(msg) {
      ...
      if (!subscription) {
        ...
        // console.log(node.time,node.timeUnit);
    ...
    async function browse_action_input(msg) {
      ...
      if (node.session) {
        ...
        for(let reference of browseResult.references) {
          if (reference.browseName.namespaceIndex === 2) {
            const dataValue = await node.session.read({nodeId: reference.nodeId, attributeId: opcua.AttributeIds.Value});
            all.push({ name: reference.browseName.name, nodeId: reference.nodeId, value: dataValue.value.value, dataType: opcua.DataType[dataValue.value.dataType] });
          }
  ...
  RED.nodes.registerType("Novakon-Client", OpcUaClientNode);
};
